...
_NPM_IMPORT_TMPL = """    npm_import(
        name = "{name}",
        integrity = "{integrity}",
        package_name = "{package_name}",
        package_version = "{package_version}",{maybe_deps}{maybe_transitive}{maybe_patches}{maybe_patch_args}
    )
"""
...
def _impl(rctx):
    ...
    repositories_bzl = [
        "# @generated by translate_pnpm_lock.bzl from {pnpm_lock}".format(pnpm_lock = str(rctx.attr.pnpm_lock)),
    ...
    for (i, v) in enumerate(packages.items()):
        ...
        repo_name = "npm_import__%s" % npm_utils.bazel_name(name, version)
    ...
    rctx.file("nodejs_packages.bzl", "\n".join(nodejs_packages_header_bzl + [""] + nodejs_packages_bzl) + "\n")

    rctx.file("BUILD.bazel", "")

    # rctx.file("repositories.bzl", header + "\n".join(_import_dependencies(lock_content, patches_map = rctx.attr.patches, patch_args_map = rctx.attr.patch_args)))
    # Allow users to refer to the generated file, e.g. from write_source_files
    # _define_aliases(rctx, lock_content)
