...
load("@aspect_bazel_lib//lib/private:copy_file.bzl", "copy_bash", "copy_cmd")
...
def _create_launcher(ctx, args):
    ...
    if ctx.file.entry_point.is_source:
        ...
        if ctx.attr.is_windows:
            copy_cmd(ctx, ctx.file.entry_point, ctx.file.entry_point.path, entry_point)
        else:
            copy_bash(ctx, ctx.file.entry_point, ctx.file.entry_point.path, entry_point)
    else:
